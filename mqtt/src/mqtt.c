...
struct timer_list pubRecvTimer;
...
void pubRecvTout( uintptr_t arg ){
  // PUBLISH не получен вовремя
  sUartRxHandle *handle = (sUartRxHandle *)arg;
  ...
  while ( len0 ){
    switch ( sim->mqttReceive.msgState ){
      case MSG_NULL:
        ...
        switch( sim->mqttReceive.msgType ) {
          case MQTT_PUBLISH:
            timerMod( &pubRecvTimer, TOUT_1000 * 30 );
            break;
          case MQTT_PINGRESP:
          case MQTT_CONNACK:
          case MQTT_SUBACK:
...
//    char str[32] = {0};
    ...
    timerSetup( &pubRecvTimer, pubRecvTout, (uintptr_t)simHnd.rxh );
}
